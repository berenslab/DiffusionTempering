using DrWatson
@quickactivate "FenrirForNeuro"
using FenrirForNeuro
using LinearAlgebra, Statistics
using ModelingToolkit
using Optim, OptimizationOptimJL

using Random

# set up experiment
# --------------------------------------------
exp_name = splitext(basename(@__FILE__))[1]
out_dir = mkoutdir(exp_name; base_dir="./")

# set random seed
Random.seed!(1234)

# construct IVP
# --------------------------------------------
@parameters gK gNa
θ = [gNa => 25, gK => 7]
prob_prior, prob = get_SinglecompartmentHH(θ)

# set up inference
# --------------------------------------------
σ² = 1e-1
dt_data = 1e-2
dt_sol = 1e-2
proj = [1 0 0 0]
data = generate_data(prob, proj, σ², dt_data)

# fenrir loss
loss = get_fenrir_loss()

# set loss parameters
prior = prob_prior
other_params = (data=data, prob=prob, prior=prior, σ²=σ², proj=proj, order=3, dt=dt_sol)

# set tempering schedule
κ²0 = 20.0
# τ = 5.0; T(t::Real)::Float64 = 10.0^(κ²0 * exp(-t / τ))
T(t::Real)::Float64 = 10.0^(κ²0 - t)
tempering_schedule = T.(LinRange(0, 20, 21))

function box_intersect(lb, ub, v1, v2)
    left = seg_intersect(lb, diag([lb ub]), v1, v2)
    bottom = seg_intersect(lb, diag([ub lb]), v1, v2)
    right = seg_intersect(diag([ub lb]), ub, v1, v2)
    top = seg_intersect(diag([lb ub]), ub, v1, v2)
    intersects = [left bottom right top]
    within_box = [all(lb .<= intersects .<= ub, dims=1)...]
    v1, v2 = eachcol(intersects[:, within_box])
    return v1, v2
end

function seg_intersect(a1, a2, b1, b2)
    perp(a) = [-a[2], a[1]]
    db, dp = b2 - b1, a1 - b1
    dap = perp(a2 - a1)
    return ((dap' * dp * db) / (dap' * db)) + b1
end

function line_through_c(c, φ)
    φ = φ * π / 180
    v1 = [c[1] - 500 * cos(φ), c[2] - 500 * sin(φ)]
    v2 = [c[1] + 500 * cos(φ), c[2] + 500 * sin(φ)]
    return v1, v2
end

function interp(h, v1, v2)
    return v1 + h .* (v2 - v1)
end

φ = 165
v1, v2 = line_through_c([25, 7], 165)
v1, v2 = box_intersect([0, 0], [80, 15], v1, v2)

ps₀ = hcat([interp(h, v1, v2) for h in LinRange(0, 1, 100)]...)'

from, to = length(ARGS) == 2 ? parse.(Int64, ARGS) : (1, size(ps₀, 1))

fout = "$out_dir" * "$exp_name-$φ-$from-$to.csv"
println("currently writing to ", fout)
for (idx, p₀) in zip(from:to, eachrow(ps₀[from:to, :]))
    println("$idx/$(to-from+1)")
    l(κ²) = loss(p₀, (other_params..., logκ²=log.(κ²)))[1]

    row = []
    push!(row, p₀...)
    for κ² in tempering_schedule
        try
            push!(row, l(κ²))
        catch e
            push!(row, NaN)
        end
    end
    vec2csv(fout, row)
end